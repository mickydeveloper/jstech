  ...
  <div class="recent-list">
    <div class="card-wrapper">
      <div class="card" v-for="(recent, index) in RecentList" v-bind:class="{'no-data': !recent.dayExists && !getTempByIndex(index) }">
        <div class="data" v-if="recent.dayExists == true && recent.dayType != '' && recent.activeLevel != '' && (!getTempByIndex(index) || !getTempByIndex(index).recent.dayExists)" v-on:click="setActiveMode(recent,index)">
          <div class="face">
        ...
        <div class="edit" v-if="getTempByIndex(index) ? getTempByIndex(index).recent.dayExists : getTempByIndex(index)">
...
export default {
  ...
  methods : {
    ...
    saveRecent(temp,recent){
      ...
      this.recentTemp.splice(this.recentTemp.indexOf(temp.day),1)
      this.$store.dispatch("postRecent").then(() => {
      })
    }
  }
}
